...
/**
 * Servlet implementation class maestriaController
 */
@WebServlet("/maestriaController")
public class maestriaController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (int i = 0; i < aux.getDireccionpros().size(); i++) {
			if (aux.getDireccionpros().get(i).getTipoPro().equalsIgnoreCase("Maestria")) {
				d.add(aux.getDireccionpros().get(i));
			}
		}
		
		
		request.getSession().setAttribute("direccionMaestria",d );
